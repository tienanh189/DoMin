...
public class Board {
    ...
    public int[] arr_R = Config.arr_Rows;
    public int[] arr_C = Config.arr_Colunms;
    public int[] arr_M = Config.arr_Mines;
    ...
    private int genRan(int range) {
        ...
        //Hàm target đánh dấu kiểm tra xem ô đang đóng không nếu đang đóng thì được phép đánh dấu
        public void target(int x, int y) {
            if (!square[x][y].isOpen()) {
                if (!square[x][y].isTarget()) { square[x][y].setTarget(true); }
                else { square[x][y].setTarget(false); }
            }
        }
        //Hàm hiển thị toàn bộ bàn chơi khi game kết thúc
        public void showAllSquares() {
            for (int i = 0; i < square.length; i++) {
                for (int j = 0; j < square[0].length; j++) { square[i][j].setOpen(true); }
            }
        }
}
